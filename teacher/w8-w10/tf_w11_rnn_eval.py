...
train_history=model.fit(x_train, y_train, validation_split=0.2,epochs=epochs, batch_size=batch_size, verbose=1,callbacks=[tensorboard_callback])
# 顯示訓練成果(分數)
...
predictions_score = model.predict(tf.expand_dims(x_test[test_sample_index], 0),batch_size=batch_size)
print("predictions_score",predictions_score)

print ("Label is ",np.where(predictions_score==np.amax(predictions_score)))
#print("Label is  ",max_array[0])
#print(predictions_score)

plt.imshow(x_test[test_sample_index],cmap=plt.cm.gray)
plt.show() 
